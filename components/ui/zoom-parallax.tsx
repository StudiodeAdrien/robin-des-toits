        ...
        >
          <div className="absolute inset-x-0 bottom-16 md:bottom-24 px-8 md:px-16">
            <div className="flex items-end justify-between gap-6">
              ...
              {/* Mobile: horizontal scrolling reviews above text */}
              <motion.div
                style={{ x: fcButtonY, opacity: fcButtonOpacity }}
                className="pointer-events-auto absolute top-20 inset-x-0 flex md:hidden gap-3 overflow-x-auto px-4 pb-2 snap-x snap-mandatory scrollbar-hide"
              >
                {[
                  { author: 'Jos Van H.', text: "M. Vincent Cardona a récemment remplacé notre toiture. Nous sommes très satisfaits du résultat." },
                  { author: 'Michèle S.', text: "Je recommande cette entreprise qui m'a donné entière satisfaction quant au devis, délai, travail impeccable." },
                  { author: 'Pascal P.', text: "Bravo pour cet excellent travail ! Un nouveau toit nous abrite en gardant son authenticité." },
                ].map((review) => (
                  <a
                    key={review.author}
                    href={GOOGLE_MAPS_URL}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="block min-w-[75vw] snap-center rounded-xl border border-white/20 bg-white/10 px-4 py-3 backdrop-blur-sm"
                  >
